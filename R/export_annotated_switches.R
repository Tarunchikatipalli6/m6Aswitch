...
#'
#' Writes annotated m6A switching results to file with optional annotation tracks
#' for IGV visualization and downstream analysis.
#'
...
export_annotated_switches <- function(m6a_switches, 
  ...
  # Export as CSV with all annotations
...
#'
#' Creates a text report summarizing m6A switching analysis.
#'
#' @param m6a_switches data.table from annotate_m6a_switches()
...
generate_summary_report <- function(m6a_switches, output_file = "m6aswitch_report.txt") {
  ...
  for (i in 1:nrow(top_genes)) {
    gene <- top_genes[i, gene_id]
    count <- top_genes[i, N]
    report <- c(report, sprintf("  %s: %d m6A events", gene, count))
  }
